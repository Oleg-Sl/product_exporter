from domain.furnitures.bed import Bed
from domain.value_objects.dimensions import Dimensions
from domain.value_objects.prices import Prices
from ..schemas.furnitures.bitrix_bed_schema import BitrixBedSchema
from ..schemas.calculations.bitrix_calculation_bed_schema import BitrixCalculationBedSchema
from ..schemas.economy import BitrixEconomySchema
from ..schemas.fot import BitrixFotSchema


class BedMapper:
    def __init__(self, collection_mapping: dict[str, str], mechanism_mapping: dict[str, str], box_mapping: dict[str, str]):
    ...
    def map_bitrix_to_domain(
            self,
            data: BitrixBedSchema,
            economy_data: BitrixEconomySchema,
            fot_data: BitrixFotSchema,
            calculation_data: BitrixCalculationBedSchema
        ) -> Bed:
    ...
    def _calc_fabric_consumption(self, calculation_data: BitrixCalculationBedSchema) -> float:
    ...
    def _get_dimensions(self, data: BitrixBedSchema) -> Dimensions:
        return Dimensions(
            width=data.width,
            height=data.height,
            depth=data.depth
    ...
    def _get_prices(self, economy_data: BitrixEconomySchema, fot_data: BitrixFotSchema, calculation_data: BitrixCalculationBedSchema) -> Prices:
        return Prices(
